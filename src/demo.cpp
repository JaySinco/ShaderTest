...
float animateLights(NodePath render, AnimControlCollection shuttersAnimationCollection, float delta,
                    float speed, bool &closedShutters, bool middayDown, bool midnightDown);
...
void showBuffer(NodePath render2d, NodePath statusNP,
                std::tuple<std::string, PT(GraphicsOutput), int> bufferTexture, bool alpha);
...
LVecBase3f calculateCameraLookAt(double upDownAdjust, double leftRightAdjust, double phi,
                                 double theta, LVecBase3 lookAt);
...
LColor mixColor(LColor a, LColor b, float amount);
...
LVecBase4f sunlightColor0 = LVecBase4f(0.612, 0.365, 0.306, 1);
LVecBase4f sunlightColor1 = LVecBase4f(0.765, 0.573, 0.400, 1);
LVecBase4f moonlightColor0 = LVecBase4f(0.247, 0.384, 0.404, 1);
LVecBase4f moonlightColor1 = LVecBase4f(0.392, 0.537, 0.571, 1);
LVecBase4f windowLightColor = LVecBase4f(0.765, 0.573, 0.400, 1);
...
int main(int argc, char *argv[])
{
    ...
    std::vector<PT(AudioSound)> sounds = {audioManager->get_sound("sounds/wheel.ogg", true),
    ...
    PT(Texture)
    colorLookupTableTextureN = TexturePool::load_texture("images/lookup-table-neutral.png");
    ...
    cameraNP.set_pos(calculateCameraPosition(cameraRotateRadius, cameraRotatePhi, cameraRotateTheta,
                                             cameraLookAt));
    ...
    NodePath shuttersNP =
        window->load_model(framework.get_models(), "eggs/mill-scene/shutters.bam");
    shuttersNP.reparent_to(sceneRootNP);
    NodePath weatherVaneNP =
        window->load_model(framework.get_models(), "eggs/mill-scene/weather-vane.bam");
    ...
    FramebufferTexture geometryFramebufferTexture0 =
        generateFramebufferTexture(framebufferTextureArguments);
    ...
    geometryBuffer0->add_render_texture(NULL, GraphicsOutput::RTM_bind_or_copy,
    ...
    geometryBuffer1->add_render_texture(NULL, GraphicsOutput::RTM_bind_or_copy,
                                        GraphicsOutput::RTP_aux_rgba_0);
    ...
    geometryBuffer2->add_render_texture(NULL, GraphicsOutput::RTM_bind_or_copy,
                                        GraphicsOutput::RTP_aux_rgba_0);
    ...
    baseBuffer->add_render_texture(NULL, GraphicsOutput::RTM_bind_or_copy,
                                   GraphicsOutput::RTP_aux_rgba_0);
    baseBuffer->set_clear_active(3, true);
    baseBuffer->set_clear_value(3, framebufferTextureArguments.clearColor);
    baseBuffer->set_sort(std::max(ssaoBlurBuffer->get_sort() + 1, UNSORTED_RENDER_SORT_ORDER + 1));
    ...
    depthOfFieldBuffer->add_render_texture(NULL, GraphicsOutput::RTM_bind_or_copy,
                                           GraphicsOutput::RTP_aux_rgba_0);
    ...
    showBuffer(render2d, statusNP, bufferArray[showBufferIndex], false);
    ...
    auto beforeFrame = [&]() -> void {
        ...
        if (keyDebounced) {
            if (tabDown) {
                ...
                bool showAlpha =
                    bufferName == "Outline" || bufferName == "Foam" || bufferName == "Fog";
            ...
            if (deleteDown) {
                ...
                toggleStatus(LVecBase2f(soundEnabled ? 1 : 0, 0), "Sound");
            }
            ...
            if (sunlightDown) {
                ...
                toggleStatus(LVecBase2f(animateSunlight ? 1 : 0, 0), "Sun Animation");
            }
        ...
        if (animateSunlight || middayDown || midnightDown) {
            sunlightP = animateLights(render, shuttersAnimationCollection, delta, -360.0 / 64.0,
                                      closedShutters, middayDown, midnightDown);
        ...
        cameraLookAt = calculateCameraLookAt(cameraUpDownAdjust, cameraLeftRightAdjust,
                                             cameraRotatePhi, cameraRotateTheta, cameraLookAt);

        cameraNP.set_pos(calculateCameraPosition(cameraRotateRadius, cameraRotatePhi,
                                                 cameraRotateTheta, cameraLookAt));
        ...
        fogNP.set_shader_input("sunPosition", LVecBase2f(sunlightP, 0));
        fogNP.set_shader_input("origin",
                               cameraNP.get_relative_point(render, environmentNP.get_pos()));
        fogNP.set_shader_input("nearFar", LVecBase2f(fogNear, fogFar));
        fogNP.set_shader_input("enabled", fogEnabled);
        fogCamera->set_initial_state(fogNP.get_state());
        ...
        refractionUvNP.set_shader_input("lensProjection",
                                        geometryCameraLens1->get_projection_mat());
        refractionUvNP.set_shader_input("enabled", refractionEnabled);
        refractionUvNP.set_shader_input("rior", rior);
        refractionUvCamera->set_initial_state(refractionUvNP.get_state());

        reflectionUvNP.set_shader_input("lensProjection",
                                        geometryCameraLens1->get_projection_mat());
        reflectionUvNP.set_shader_input("enabled", reflectionEnabled);
        reflectionUvCamera->set_initial_state(reflectionUvNP.get_state());
    ...
    auto setMouseWheelUp = [&]() { mouseWheelUp = true; };

    auto setMouseWheelDown = [&]() { mouseWheelDown = true; };

    framework.define_key(
        "wheel_up", "Mouse Wheel Up",
        [](const Event *, void *arg) { (*static_cast<decltype(setMouseWheelUp) *>(arg))(); },
        &setMouseWheelUp);

    framework.define_key(
        "wheel_down", "Mouse Wheel Down",
        [](const Event *, void *arg) { (*static_cast<decltype(setMouseWheelDown) *>(arg))(); },
        &setMouseWheelDown);

    physicsManager.attach_linear_integrator(new LinearEulerIntegrator());

    LVector3f wheelNPRelPos = wheelNP.get_pos(sceneRootNP);
    sounds[0]->set_3d_attributes(wheelNPRelPos[0], wheelNPRelPos[1], wheelNPRelPos[2], 0, 0, 0);
    LVector3f waterNPRelPos = waterNP.get_pos(sceneRootNP);
    sounds[1]->set_3d_attributes(waterNPRelPos[0], waterNPRelPos[1], waterNPRelPos[2], 0, 0, 0);
                    ...
                    float speed, bool &closedShutters, bool middayDown, bool midnightDown)
{
    ...
    PT(DirectionalLight) sunlight = DCAST(DirectionalLight, sunlightNP.node());
    PT(DirectionalLight) moonlight = DCAST(DirectionalLight, moonlightNP.node());
...
PTA_LVecBase3f generateSsaoSamples(int numberOfSamples)
{
    ...
    for (int i = 0; i < numberOfSamples; ++i) {
        LVecBase3f sample = LVecBase3f(randomFloats(generator) * 2.0 - 1.0,
    ...
    FramebufferTextureArguments framebufferTextureArguments)
{
    ...
    buffer = graphicsEngine->make_output(
        graphicsOutput->get_pipe(), name + "Buffer", BACKGROUND_RENDER_SORT_ORDER - 1, fbp,
        WindowProperties::size(0, 0),
        GraphicsPipe::BF_refuse_window | GraphicsPipe::BF_resizeable |
            GraphicsPipe::BF_can_bind_every | GraphicsPipe::BF_rtt_cumulative |
...
void hideBuffer(NodePath render2d)
{
    ...
    if (nodePath) nodePath.detach_node();
}
...
void setSoundOff(PT(AudioSound) sound) { setSoundState(sound, false); }

void setSoundOn(PT(AudioSound) sound) { setSoundState(sound, true); }
...
void updateAudoManager(NodePath sceneRootNP, NodePath cameraNP)
{
    ...
    audioManager->audio_3d_set_listener_attributes(p[0], p[1], p[2], v[0], v[1], v[2], f[0], f[1],
                                                   f[2], u[0], u[1], u[2]);
...
NodePath setUpParticles(NodePath render, PT(Texture) smokeTexture)
{
    ...
    smokeSPR->get_color_interpolation_manager()->add_linear(0.0, 1.0, LColor(1.0, 1.0, 1.0, 1.0),
                                                            LColor(0.039, 0.078, 0.156, 1.0), true);
...
double microsecondToSecond(int m) { return m / 1000000.0; }
...
LColor mixColor(LColor a, LColor b, float factor) { return a * (1 - factor) + b * factor; }
