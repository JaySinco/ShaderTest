...
void main()
{
    ...
    vec4 normalTex =
        texture(p3d_Texture1, vec2(normalCoord.x + flowMapsEnabled.x * flow.x * osg_FrameTime,
    ...
    if (isParticle.x == 1) {
        ...
        normal = normalize(mat3(tangent, binormal, vertexNormal) * normalRaw);
    } else {
    ...
    for (int i = 0; i < p3d_LightSource.length(); ++i) {
        vec3 lightDirection =
            p3d_LightSource[i].position.xyz - vertexPosition.xyz * p3d_LightSource[i].position.w;
        ...
        float attenuation =
            1.0 / (p3d_LightSource[i].constantAttenuation +
        ...
        diffuseIntensity =
            celShadingEnabled.x == 1 ? smoothstep(0.1, 0.2, diffuseIntensity) : diffuseIntensity;

        vec4 lightDiffuseColor = p3d_LightSource[i].diffuse;
        lightDiffuseColor.rgb = pow(lightDiffuseColor.rgb, vec3(gamma.x));

        vec4 diffuseTemp =
            vec4(clamp(diffuseColor.rgb * lightDiffuseColor.rgb * diffuseIntensity, 0.0, 1.0),
                 diffuseColor.a);

        float specularIntensity =
            (blinnPhongEnabled.x == 1 ? clamp(dot(normal, halfwayDirection), 0.0, 1.0)
                                      : clamp(dot(eyeDirection, reflectedDirection), 0.0, 1.0));

        specularIntensity = (celShadingEnabled.x == 1 ? smoothstep(0.9, 1.0, specularIntensity)
                                                      : specularIntensity);
        ...
        if (fresnelEnabled.x == 1) {
            float fresnelFactor =
                dot((blinnPhongEnabled.x == 1 ? halfwayDirection : normal), eyeDirection);
            ...
            materialSpecularColor.rgb =
                mix(materialSpecularColor.rgb, vec3(1.0), clamp(fresnelFactor, 0.0, 1.0));
        }

        vec4 specularTemp = vec4(vec3(0.0), diffuseColor.a);
        specularTemp.rgb =
            lightSpecularColor.rgb * pow(specularIntensity, specularMap.g * MAX_SHININESS);
        ...
        float unitLightDirectionDelta =
            dot(normalize(p3d_LightSource[i].spotDirection), -unitLightDirection);
        ...
        for (int si = -shadowSamples; si <= shadowSamples; ++si) {
            for (int sj = -shadowSamples; sj <= shadowSamples; ++sj) {
                inShadow += (1.0 - textureProj(p3d_LightSource[i].shadowMap,
                                               vertexInShadowSpaces[i] +
                                                   vec4(vec2(si, sj) / shadowMapSize, vec2(0.0))));
        ...
        vec3 shadow = mix(vec3(1.0), shadowColor, inShadow);
    ...
    if (rimLightEnabled.x == 1) {
        rimLight.rgb = vec3(1.0 - max(0.0, dot(normalize(-vertexPosition.xyz), normalize(normal))));
        rimLight.rgb = (celShadingEnabled.x == 1 ? smoothstep(0.3, 0.4, rimLight.rgb)
    ...
    out0.rgb = ambient.rgb + diffuse.rgb + rimLight.rgb + emission.rgb;
